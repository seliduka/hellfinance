...
########################################################
CG <- getSymbols("9955.tw", auto.assign = FALSE, from = "2017-01-01")
...
CG <- cbind(symbol = "回收-佳龍", CG)
CG$date <- as.Date(CG$date, format =  "%Y-%m-%d")
CG <- as.tibble(CG)


UD <- getSymbols("1101.tw", auto.assign = FALSE, from = "2017-01-01")
